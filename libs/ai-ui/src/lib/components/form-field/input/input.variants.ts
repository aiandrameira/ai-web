...
export const inputVariants = cva("block w-full text-sm text-muted-foreground appearance-none focus:outline-none focus:ring-0 peer", {
    variants: {
        variant: {
...
export const labelVariants = cva("text-sm text-muted-foreground duration-300", {
    variants: {
        variant: {
            outlined:
                "absolute origin-[0] pointer-events-none -translate-y-4 scale-75 top-2 z-10 bg-background px-2 start-3 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 peer-focus:text-primary",
            standard:
